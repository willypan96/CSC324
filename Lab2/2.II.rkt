...
(check-expect (flatten (list (list (list 1 2) (list 3 4))
              ...
              (apply append (list (apply append (list (list 1 2) (list 3 4)))
                              (apply append (list (list 1 3) (list 2))))))

; If you didn't recurse in the previous part, do so now:
(check-expect (flatten (list (list (list 1 2) (list 3 4))
                             (list (list 1 3) (list 2))))
              (apply append (list (flatten (list (list 1 2) (list 3 4)))
                              (flatten (list (list 1 3) (list 2))))))
...
; Implement flatten.
#|
The idea is to recursively define this function.
If there is an element that isn't a list, then we can make it a list (in fact it will be flattenned because a list of one element is guaranteed to be flattenned)
If the element we care about is empty, then we return the empty list because a flattenned list of an empty list is just the empty list. Also
note that on append, appending an empty list to other lists just creates the same original list.
Otherwise, we proceed to flatten all the elements of v. By inductive hypothesis, we have these recursive calls to map work, so we now have a list of flattened elements, so all we do is apply append to
all the elements of this list of flattenned lists.

Proof by induction to come.
|#
(define (flatten v)
  (if (not (list? v))
      (list v)
